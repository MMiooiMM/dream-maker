...
- **🎬 場景頁籤** — 依世界觀提供場景選項（場景選項由 shared JSON 管理），支援一鍵生成 5 個場景；使用者只需命名場景與設定角色身份
...
4. **設定場景** — 點擊一鍵生成，系統依世界觀產出 5 個場景選項；你只需調整場景名稱與角色身份
5. **調整基調** — 設定虐度/爽度、誤會強度、結局類型；「誰做錯事」會依目前配對與 ABO 顯示角色稱呼，且若單方做錯，僅可設定另一方（被辜負者）的回頭意願
6. **編排 12 章** — 從區塊庫拖曳事件到章節，或使用一鍵生成
7. **檢查節奏** — 查看情緒曲線與系統提醒
8. **匯出** — 下載 Markdown 框架、JSON 或 PNG 截圖
