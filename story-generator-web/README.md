...
- **👤 角色創建** — 性格特質點數分配、資源等級條、關係設定，配角可選身分與對主角初始關係
...
3. **創建角色** — 設定男主/女主的姓名、身分、性格特質、資源；配角可透過下拉選單設定身分、關聯主角與初始關係
4. **調整基調** — 設定虐度/爽度、誤會強度、結局類型；「誰做錯事」會依目前配對與 ABO 顯示角色稱呼，且若單方做錯，僅可設定另一方（被辜負者）的回頭意願
