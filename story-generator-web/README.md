...
- **🎬 場景頁籤** — 可建立多個場景、撰寫符合世界觀的場景內容，並設定角色在場景中的定位
- **🧩 互動物件設計** — 每個場景可配置可互動物件與互動方式，讓使用場景時更具畫面感
- **🎭 豐富基調設定** — 基調頁籤會依配對模式與 ABO 第二性別動態顯示稱呼；若單方做錯，回頭焦點自動切換為另一方
...
4. **設定場景** — 新增場景、填寫世界觀契合描述與場景內容，並指定角色定位
5. **配置互動物件** — 為各場景加入可互動物件（名稱 + 互動方式）
6. **調整基調** — 設定虐度/爽度、誤會強度、結局類型；「誰做錯事」會依目前配對與 ABO 顯示角色稱呼，且若單方做錯，僅可設定另一方（被辜負者）的回頭意願
7. **編排 12 章** — 從區塊庫拖曳事件到章節，或使用一鍵生成
8. **檢查節奏** — 查看情緒曲線與系統提醒
9. **匯出** — 下載 Markdown 框架、JSON 或 PNG 截圖
